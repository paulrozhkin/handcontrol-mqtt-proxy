/*
 * crc8.c
 *
 *  Created on: Nov 21, 2020
 *      Author: Pavel Rozhkin
 */

#include "crc8.h"
#include <stdint.h>
#include <stddef.h>

#include <stdio.h>
#include <stdlib.h>

// 0x07 Polynomial Table, 8-bit, http://www.sunshine2k.de/coding/javascript/crc/crc_js.html
static const uint8_t crc_table[] = {
		  };

uint8_t fast_crc_cal8bits(uint8_t crc, int size, const uint8_t *data)
{
  while(size--)
  {
    crc = crc ^ *data++; // Apply Byte

    crc = crc_table[crc & 0xFF]; // One round of 8-bits
  }

  return(crc);
}

uint8_t calculate_crc8(const uint8_t* data, size_t length, uint8_t crc_input)
{
	uint8_t crc = fast_crc_cal8bits(crc_input, length, data);
	return crc;
}
